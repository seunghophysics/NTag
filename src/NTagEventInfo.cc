...
fVerbosity(vDefault), bData(false) { bonsai_ini_(); }
...
{
	std::array<float, 3> trueCaptureVertex;
	float tRecon = ReconCaptureTime(capID);
...
float NTagEventInfo::SubtractToF(float t_ToF[], float T[], int PMTID[], float vertex[3], bool doSort)
{
    ...
    for(int iHit = 0; iHit < nHits; iHit++){
        ...
        float ToF = Norm(vecFromVertexToPMT) / C_WATER;
...
float NTagEventInfo::MinimizeTRMS(float T[], int PMTID[], float rmsFitVertex[])
{
    ...
    float tVX, tVY, tVZ;	// temp vertex
	float sVX, sVY, sVZ;	// loop search vertex
        ...
        for(float x = 0; x < maxScanR; x++){
            ...
            for(float y = 0; y < maxScanR; y++){
                ...
                for(float z = 0; z < maxScanZ; z++){
					...
					float vecFromVtoSV[3];
					vecFromVtoSV[0] = sVX - vx; vecFromVtoSV[1] = sVY - vy; vecFromVtoSV[2] = sVZ - vz;
					if(Norm(vecFromVtoSV) > distanceCut) continue;

					float t_ToF[nHits];
					float sV[3]; sV[0] = sVX; sV[1] = sVY; sV[2] = sVZ;
					tRMS = SubtractToF(t_ToF, T, PMTID, sV, true);
